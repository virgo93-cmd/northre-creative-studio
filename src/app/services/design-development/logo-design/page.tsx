...
import { logoDesignContent } from "@/config/services/logo-design";

export const metadata = {
  title: "Logo Design - NORTHRE CREATIVE STUDIO",
  description: "Perancangan identitas visual dan konstruksi geometri logo yang ikonik serta timeless.",
};

// Objek pemetaan Ikon SVG khusus untuk mempercantik judul kustom tiap pilar layanan logo
const pillarIcons: Record<string, React.ReactNode> = {
  ...
  "pillar-3": (
    <svg className="w-5 h-5 text-neutral-500 group-hover:text-white transition-colors duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5c-.345 3.522-1.763 6.642-3.939 9" />
    </svg>
  ),
  "pillar-4": (
    <svg className="w-5 h-5 text-neutral-500 group-hover:text-white transition-colors duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
// Judul pilar kustom profesional untuk memperkuat sektor desain identitas logo
const attractivePillarTitles: Record<string, string> = {
  "pillar-1": "Corporate & Brand Identity",
  "pillar-2": "Visual System Guidelines",
  "pillar-3": "Typography & Lettering Design",
  "pillar-4": "Collateral Brand Mockups",
};

export default function LogoDesignPage() {
  const { hero, pillars, benefits, workflow, portfolio } = logoDesignContent;
        ...
        <section className="relative w-full min-h-120 flex items-center rounded-3xl overflow-hidden border border-neutral-900/40 bg-black shadow-2xl p-8 md:p-16 group">
          ...
          <div className="absolute inset-y-0 right-0 w-full lg:w-1/2 h-full z-0 pointer-events-none overflow-hidden">
            <div className="relative w-full h-full opacity-40 group-hover:opacity-50 transition-opacity duration-700">
              <Image
                src={hero.imagePath}
                alt="Northre Logo Design Hero Placeholder Visual"
                fill
                priority
